...
> ⚠️ **Статус проекта:** Находится в активной разработке. Текущая версия содержит полностью рабочую Core-логику: CRUD для целей мониторинга, автоматический фоновый scheduler с worker pool, систему инцидентов, кэширующий rate-limiter и интеграцию с Telegram-ботом.
...
| `timeout_seconds` | Таймаут HTTP-запроса. От `1` до `30` секунд. Должен быть минимум в 2 раза меньше интервала |
...
Проект поддерживает уведомления об инцидентах с помощью Telegram-бота.
Для работы уведомлений необходимо вручную заполнить поля: `TELEGRAM_BOT_TOKEN` и `TELEGRAM_CHAT_ID` в .env

Если одно из полей или оба не будут заполнены, то приложение корректно запустится, записав предупреждение о том, что уведомления выключены. В таком случае система не будет отправлять уведомления, но будет продолжать корректно обрабатывать инциденты.

Уведомление хранит в себе следующие данные:
- Имя целевого сервиса
...
- Тестами покрыта лишь небольшая часть.

## Планы по доработке

- Покрыть большую часть сервиса тестами.
